...
Humanisti::Humanisti(QList<QPointF> pointsToFollow, QGraphicsItem *parent):
    EnemyBase(pointsToFollow, parent)
{
    // set graphics for enemy
    setPixmap(QPixmap(":/images/humanisti.png"));
...
double Humanisti::getHealth()
{
    return health_;
}

void Humanisti::moveForward()
{
...
double Humanisti::getDamage()
{
    return damage_;
}

double Humanisti::setHealth(double damage)
{
    if (health_ <= 0)
    {
        delete this;
