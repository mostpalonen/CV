...
Feministi::Feministi(QList<QPointF> pointsToFollow, QGraphicsItem *parent):
    EnemyBase(pointsToFollow, parent)
{
    // set graphics for enemy
    setPixmap(QPixmap(":/images/feministi.png"));
...
double Feministi::getHealth()
{
    return health_;
}

void Feministi::moveForward()
{
...
double Feministi::getDamage()
{
    return damage_;
}

double Feministi::setHealth(double damage)
{
    if (health_ <= 0)
    {
        delete this;
